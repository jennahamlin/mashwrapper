# nf-core/mashwrapper: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/mashwrapper/usage](https://nf-co.re/mashwrapper/usage)
...
```console
nextflow run nf-core/mashwrapper --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/mashwrapper
```
...
First, go to the [nf-core/mashwrapper releases page](https://github.com/nf-core/mashwrapper/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
